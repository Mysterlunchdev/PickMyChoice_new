...
class Social_Mdl {
    ...
    function get_vendors_list($ulat,$ulong,$cat_id=NULL)
    {
        ...
        ( 3959 * acos ( cos ( radians('$ulat') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians($ulong) ) + sin ( radians('$ulat') ) * sin( radians( latitude ) ) ) ) AS `distance` 
         FROM `user` where department_id='3' and longitude!='' and latitude!='' order by distance";
        $data =  $this->db->executeQuery($sql);
        return $data;
    }
    ...
    {
      $sql = "select s.id,s.category_id,s.name as sname,IFNULL(s.description,'') as description,IFNULL(s.attachment,'') as attachment,c.name as cname from subcategory s inner join category c
    ...
    function update_profile($user_id,$flag)
    {
        if($flag=='personal')
        {
            $sql="update user set name='".$this->name."',last_name='".$this->last_name."',email='".$this->email."',gender='".$this->gender."',
            dob='".$this->dob."',postcode='".$this->post_code."',address='".$this->address."',city='".$this->city."',street='".$this->street."'";
            if($this->photo!='')
            {
...
/* Function Start Date: 30-12-2020, Written By: Madhavi B, Purpose: For task and image both will add using this function.*/
	function add_task_web()
     {
		...
		if($taskid!="")
		{
			 ...
			 `file_name`,`attachment`,`status`,`log_date_created`,`created_by`) values("job","image","'.$taskid.'","Task Image","'.$this->attachment.'","Active","'.$dttime.'","'.$this->user_id.'")';
    ...
    {
        $date=date('Y-m-d H:i:s');
    	$sql = 'insert into user (department_id,name,last_name,user_code,email,mobile,gender,dob,mobile_verified,fb_id,google_id,postcode,address,city,street,profile_photo,status,log_date_created)values
    	("3","' .$this->firstname. '","' .$this->lastname. '","' . $this->user_code . '","' . $this->email. '","' . $this->mobile . '","' . $this->gender . '","' . $this->dob .'",
    	"No","","","'.$this->post_code.'","'.$this->address.'","'.$this->city.'","'.$this->landmark.'","","Active","'.$date.'")';
    	return $this->db->getinsertidQuery($sql);
    ...
    {
        $sql ='SELECT * FROM user u LEFT JOIN department d ON d.id=u.department_id 
