...
function changeProfilePic() {
	...
	if(strlen($name)) {
		...
		if(in_array($ext,$valid_formats)) {
			if($size<(6048576)) {
				$actual_image_name = time().'_'.$_SESSION['user_id'].'.'.$ext;
...
function saveProfilePic(){
	...
	include_once("cropping/db_connect.php");
	$post = isset($_POST) ? $_POST: array();	
	//Handle profile picture update with MySQL update Query using $options array	
	if($post['id'])
	{
		$sql_query = "SELECT * FROM user WHERE id = '".mysqli_escape_string($conn, $post['id'])."'";		
		$resultset = mysqli_query($conn, $sql_query) or die("database error:". mysqli_error($conn));		
		if(mysqli_num_rows($resultset)) 
		{                     
			$sql_update = "UPDATE user set profile_photo='".mysqli_escape_string($conn,$post['image_name'])."' WHERE id = '".mysqli_escape_string($conn, $post['id'])."'";			
...
/* Function to update image */
 function saveProfilePicTmp() {
	...
	if(isset($_POST['t']) and $_POST['t'] == "ajax") {
		extract($_POST);		
		$imagePath = '../uploads/user/'.$_POST['image_name'];
		$ratio = ($t_width/$w1); 
		$nw = ceil($w1 * $ratio);
		$nh = ceil($h1 * $ratio);
