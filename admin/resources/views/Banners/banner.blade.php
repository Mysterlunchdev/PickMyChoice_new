...
@if(session()->get('userdata')[0]->department_id==1 || isset($page_access[0]->can_add) && $page_access[0]->can_add==1)
		<a href="{{URL::to('add-banner')}}" class="btn btn-primary">
	...
	<link rel="stylesheet" href="https://demo.neontheme.com/assets/js/datatables/datatables.css" id="style-resource-1">
			...
			<tbody>
				...
				@if($bannerdata->count()>0)
				 ...
				 <tr>
					...
					<td><img src="{{URL::asset('../uploads/banners/'.$data->attachment)}}" style="width:90px;"></td>
			        ...
			        <td>
						...
						<a href="{{URL::to('edit-banner',base64_encode($data->id))}}" class="btn btn-default btn-sm btn-icon icon-left">
		...
		<script type="text/javascript">
			...
			{
				$.post("{{URL::to('/changestatus')}}",{ai:ai,s:s,t:'b'},function(response){
