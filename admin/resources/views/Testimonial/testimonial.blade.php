...
@if(session()->get('userdata')[0]->department_id==1 || isset($page_access[0]->can_add) && $page_access[0]->can_add==1)
		<a href="{{URL::to('add-testimonial')}}" class="btn btn-primary">
				...
				@if($testimonial->count()>0)
				 ...
				 <tr>
					...
					<td>
						...
						<a href="{{URL::to('edit-testimonial',base64_encode($data->id))}}" class="btn btn-default btn-sm btn-icon icon-left">
		...
		<script type="text/javascript">
			...
			{
				$.post("{{URL::to('/changestatus')}}",{ai:ai,s:s,t:'testi'},function(response){
