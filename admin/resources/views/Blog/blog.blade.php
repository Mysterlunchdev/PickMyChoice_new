...
@if(session()->get('userdata')[0]->department_id==1 || isset($page_access[0]->can_add) && $page_access[0]->can_add==1)
		<a href="{{URL::to('add-blog')}}" class="btn btn-primary">
	...
	<br>
	  <br>
		<table class="table table-bordered table-striped datatable" id="table-2">
			...
			<tbody>
				...
				<tr>
					<td>{{$i++}}</td>
					<td>{{$data->title}}</td>
					<td><img src="{{URL::asset('../uploads/blog/'.$data->attachment)}}" style="width:90px;"></td>
					...
					<td>
						...
						<a href="{{URL::to('edit-blog',base64_encode($data->id))}}" class="btn btn-default btn-sm btn-icon icon-left">
		...
		<script type="text/javascript">
			...
			{
				$.post("{{URL::to('/changestatus')}}",{ai:ai,s:s,t:'bl'},function(response){
