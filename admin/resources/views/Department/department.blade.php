...
@if(session()->get('userdata')[0]->department_id==1 || isset($page_access[0]->can_add) && $page_access[0]->can_add==1)
		<a href="{{URL::to('add-department')}}" class="btn btn-primary">
				...
				@if($deptdata->count()>0)
				 ...
				 <tr>
					...
					<td>
						...
						<a href="{{URL::to('edit-department',base64_encode($data->id))}}" class="btn btn-default btn-sm btn-icon icon-left">
		...
		<script type="text/javascript">
			...
			{
				$.post("{{URL::to('/changestatus')}}",{ai:ai,s:s,t:'d'},function(response){
